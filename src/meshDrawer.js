...
void main()
{
    ...
    vec3 ambient = ambientStrength * vec3(1.0, 1.0, 1.0); // Ambient color
    float diffIntensity = max(dot(normal, lightDir), 0.0);
    vec3 diffuse = diffIntensity * vec3(1.0, 1.0, 1.0); // Diffuse color
    vec3 viewDir = normalize(-fragPos); // Assuming the view position is at the origin
    vec3 reflectDir = reflect(-lightDir, normal);
    float specIntensity = pow(max(dot(viewDir, reflectDir), 0.0), 8.0); // Using phongExp for shininess
    vec3 specular = specIntensity * vec3(1.0, 1.0, 1.0); // Specular color
    vec3 result = ambient + diffuse + specular;
    vec3 finalColor = result * texture2D(tex, vTexCoord).rgb;
    if (isLightSource) {
        gl_FragColor = vec4(texture2D(tex, vTexCoord).rgb, 1.0);
    } else {
        gl_FragColor = vec4(finalColor, 1.0); // Set the fragment color
    }
}
`;